...
const AccountSetting = ({ }: Props) => {
  ...
  const navigate = useNavigate(); // Hook để điều hướng route
  const location = useLocation(); // Hook để lấy thông tin về location hiện tại
  ...
  const settings = [
    {
      ...
      route: ROUTES.USER_PROFILE, // Đường dẫn con mà không cần "account/"
    },
    {
      icon: <FaLock />,
      ...
      <div className='content flex flex-wrap justify-center gap-6'>
        {settings.map((setting, index) => (
          <CardSetting
            ...
            onClick={() => navigate(`${location.pathname}/${setting.route}`)} // Điều hướng tới route tương ứng khi click
          />
        ))}
      </div>
    </div>
