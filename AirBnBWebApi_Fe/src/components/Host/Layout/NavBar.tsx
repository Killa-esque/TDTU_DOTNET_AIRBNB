...
const NavBar = () => {
  ...
  const items: MenuProps['items'] = [
    {
      key: '1',
      label: (
        <Link to={`${ROUTES.HOST_HOME}/${ROUTES.HOST_REVENUE}`} className="flex items-center gap-2 hover:text-pink-500">
    ...
    },
    {
      key: '2',
      label: (
        <Link to={`${ROUTES.HOST_HOME}/${ROUTES.HOST_AIRBNB_MANAGEMENT}`} className="flex items-center gap-2 hover:text-pink-500">
          <PlusCircleOutlined className="text-lg text-gray-500" />
    ...
    },
    {
      key: '3',
      label: (
        <Link to={`${ROUTES.HOST_HOME}/${ROUTES.HOST_ANALYSIS}`} className="flex items-center gap-2 hover:text-pink-500">
  ...
  return (
    <nav className="hidden md:flex items-center gap-10">
      <Link
        to={ROUTES.HOST_HOME}
        className={`hover:underline ${isActive('host') ? 'text-white font-semibold' : 'text-gray-400'}`}
      >
        Today
      </Link>
      <Link
        to={`${ROUTES.HOST_HOME}/${ROUTES.HOST_LISTINGS}`}
        className={`hover:underline ${isActive(ROUTES.HOST_LISTINGS) ? 'text-white font-semibold' : 'text-gray-400'}`}
      >
        Listings
      </Link>
      <Link
        to={`${ROUTES.HOST_HOME}/${ROUTES.HOST_MESSAGES}`}
        className={`hover:underline ${isActive(ROUTES.HOST_MESSAGES) ? 'text-white font-semibold' : 'text-gray-400'}`}
      >
        Messages
      </Link>
      <Dropdown
