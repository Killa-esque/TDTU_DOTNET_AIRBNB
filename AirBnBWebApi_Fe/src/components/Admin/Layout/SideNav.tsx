...
function SideNav({ }: Props) {
  ...
  const menuItems = [
    ...
    },
    {
      category: "Financial", items: [
        { key: "revenue", icon: <AiOutlineDollarCircle />, label: "Revenue", path: "/admin/revenue" },
      ],
    },
    {
      category: "Support", items: [
        { key: "support-management", icon: <AiOutlineQuestionCircle />, label: "Support", path: "/admin/support-management" },
      ],
    },
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-white shadow-lg">
      {/* Header của Sidebar */}
      <div className="flex items-center space-x-2 bg-pinkCustom text-white px-4 py-5">
            ...
            {group.items.map((item) => (
              <div
                ...
                className={`my-1 flex items-center space-x-4 px-4 py-3 rounded-lg cursor-pointer transition-all duration-300 ${activeKey === item.key
                  ? "bg-pinkCustom text-white shadow-md"
                  : "text-gray-700 hover:bg-pinkCustom hover:bg-opacity-20 hover:text-pinkCustom"
                  }`}
              ...
              >
                <button
                  className={`p-2 rounded-full transition-all duration-300 ${activeKey === item.key
                    ...
                    : "bg-pinkCustom text-white hover:bg-white hover:text-pinkCustom"
                    }`}
                >
